...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Como eres tan malo?', 0.1],
		['Eres muy malo!', 0.2], //From ?% to 19%
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'pico' | 'blammed' | 'philly-nice' | 'carol-roll' | 'body' | 'boogie':
				curStage = 'philly';
            ...
            case 'ugh' | 'guns' | 'stress':
				...
				var tankClouds:BGSprite = new BGSprite('tankClouds', -700, -100, 0.1, 0.1);
				add(tankClouds);
			...
			case 'light-it-up' | 'ruckus' | 'target-practice':
				...
				var bgRail:BGSprite = new BGSprite('railing', -600, 320, 0.9, 0.9);
				bgRail.active = false;
			...
			case 'sporting' | 'boxing-match':
				defaultCamZoom = 0.9;
				curStage = 'arenanight';
				var bg:BGSprite = new BGSprite('boxingnight1', -600, -200, 0, 0);
				bg.setGraphicSize(Std.int(bg.width * 1));
				bg.active = false;
				add(bg);
		
				var bg2:BGSprite = new BGSprite('boxingnight2', -600, -200, 1.2, 1.2);
				bg2.setGraphicSize(Std.int(bg.width * 1.3));
				bg2.active = false;
				add(bg2);
		
				var bg3:BGSprite = new BGSprite('boxingnight3', -600, -400, 1.1, 1.1);
			...
			case 'fisticuffs':
				defaultCamZoom = 0.9;
				curStage = 'boxFC';
				var boxwall:BGSprite = new BGSprite('boxwall', -400, -200, 0.9, 0.9, ['wallboom'], true);
				add(boxwall);
				
				var boxfloor:BGSprite = new BGSprite('boxfloor', -400, -250, 0.9, 0.9, ['bg instance 1'], true);
			...
			case 'tko':
				defaultCamZoom = 0.9;
				curStage = 'boxTKO';
				var whitewall:BGSprite = new BGSprite('backwhite', -400, -200, 0.9, 0.9);
				whitewall.active = false;
				add(whitewall);
				
				var floorblack:BGSprite = new BGSprite('floorblack', -400, -250, 0.9, 0.9);
			...
			case 'king-hit-fefe':
				defaultCamZoom = 0.9;
				curStage = 'boxKHF';
				var backtroll:BGSprite = new BGSprite('backtroll', -400, -200, 0.9, 0.9);
				backtroll.active = false;
				add(backtroll);
				
				var floortroll:BGSprite = new BGSprite('floortroll', -400, -250, 0.9, 0.9);
				floortroll.active = false;
				add(floortroll);

			case 'norway' | 'tordbot':
				curStage = 'eddhouse';
				var sky:BGSprite = new BGSprite('sky', -162.1, -386.1, 0.5, 0);
				sky.animation.addByPrefix('bg_sky1', 'bg_sky1');
				sky.animation.addByPrefix('bg_sky2', 'bg_sky2');
				if(curSong == 'Norway'){
				sky.animation.play('bg_sky1');
				}else if (curSong == 'Tordbot'){
				sky.animation.play('bg_sky2');
				}
				
				
				var bg:BGSprite = new BGSprite('bgFront', -162.1, -386.1, 0.9, 0.9);
				...
				if(curSong == 'Tordbot')
				{
					var sky:BGSprite = new BGSprite('sky', -162.1, -386.1, 0, 0);
				}
				bg.active = false;
				sky.active = false;
				bg.updateHitbox();
			...
			case 'headache' | 'nerves':
				...
				var bg:BGSprite = new BGSprite('garStagebg', -500, -170, 0.7, 0.7);
				bg.active = false;
				add(bg);
  
				var bgAlley:BGSprite = new BGSprite('garStage', -500, -200, 0.9, 0.9);
			...
			case 'release':
				...
				var bg:BGSprite = new BGSprite('garStagebgAlt', -500, -170, 0.7, 0.7);
				bg.active = false;
				add(bg);
  
				var smoker:BGSprite = new BGSprite('garSmoke', 0, -290, 0.7, 0.7, ['smokey'], true);
				smoker.setGraphicSize(Std.int(smoker.width * 1.7));
				smoker.alpha = 0.3;
				smoker.animation.play('smokey');
				add(smoker);
  
				var bgAlley:BGSprite = new BGSprite('garStagealt', -500, -200, 0.9, 0.9);
				bgAlley.active = false;
				add(bgAlley);
  
				var corpse:BGSprite = new BGSprite('gardead', -230, 540, 0.9, 0.9);
				corpse.active = false;
				add(corpse);
			...
			case 'fading':
				...
				var bg:BGSprite = new BGSprite('garStagebgRise', -500, -170, 0.7, 0.7);
				bg.active = false;
				add(bg);
  
				var bgAlley:BGSprite = new BGSprite('garStageRise', -500, -200, 0.9, 0.9);
				bgAlley.active = false;
				add(bgAlley);
			...
			case 'kitchen-shelf' | 'face':
				defaultCamZoom = 0.8;
				curStage = 'mattroom';
				var bg:BGSprite = new BGSprite('bedroomwall', -530, -452, 0.9, 0.9);
				bg.setGraphicSize(Std.int(bg.width * 0.85));
				bg.active = false;
				add(bg);
	
				var brfloor:BGSprite = new BGSprite('bedroomfloor', -650, -290, 0.9, 0.9);
				brfloor.setGraphicSize(Std.int(brfloor.width * 0.7));
				brfloor.active = false;
				add(brfloor);
	
				var bgjunk:BGSprite = new BGSprite('backgroundjunk', -350, -50, 0.9, 0.9);
				bgjunk.scale.set(0.8, 0.8);
				bgjunk.active = false;
				add(bgjunk);

			case 'reflection':
				defaultCamZoom = 0.65;
				curStage = 'mattscastle';
				var bg:BGSprite = new BGSprite('castle_bg', -970, -452, 0.9, 0.9);
				bg.active = false;
				add(bg);

				var castlefloor:BGSprite = new BGSprite('castle_floor', -780, 800, 0.9, 0.9);
				castlefloor.setGraphicSize(Std.int(castlefloor.width * 0.8));
				castlefloor.active = false;
				add(castlefloor);

				var cpoles:BGSprite = new BGSprite('poles_ig', -530, -690, 0.9, 0.9);
				cpoles.setGraphicSize(Std.int(cpoles.width * 0.9));
				cpoles.active = false;
				add(cpoles);

				var cloth:BGSprite = new BGSprite('behind_cloth', -610, -662, 0.9, 0.9);
				cloth.active = false;
				add(cloth);

				clones = new BGSprite('clone_fucks', 32, 105, 0.9, 0.9, ['clones istanza 1'], false);
				clones.updateHitbox();
				add(clones);

				edd = new BackgroundE(300, 160);
				edd.scrollFactor.set(0.9, 0.9);
				edd.updateHitbox();
				add(edd);

				tom = new BackgroundT(890, 255);
				tom.scrollFactor.set(1.0, 1.0);
				tom.updateHitbox();
				add(tom);

				/*edd = new FlxSprite(-150, -50);
					edd.frames = Paths.getSparrowAtlas('totally_not_edd');
					edd.animation.addByPrefix('bop', 'edd idle animation', 24, false);
					edd.antialiasing = true;
					edd.scrollFactor.set(0.9, 0.9);
					edd.updateHitbox();
					add(edd);

					tom = new FlxSprite(150, -50);
					tom.frames = Paths.getSparrowAtlas('totally_not_tom');
					tom.animation.addByPrefix('bop', 'tom idle animation', 24, false);
					tom.antialiasing = true;
					tom.scrollFactor.set(0.9, 0.9);
					tom.updateHitbox();
					add(tom); */

			case 'gimme-a-break' | 'man-on-stilts' | 'knock-knock':
				defaultCamZoom = 0.9;
				curStage = 'cg5stage';
		
				bg = new BGSprite('mixroom', -535, -166, 1, 0.9);
			...
			case 'screenplay' | 'parasite':
				...
				var white:BGSprite = new BGSprite('Background', 55, 250, 1, 1);
				white.screenCenter();
				add(white);
		
				var void:BGSprite = new BGSprite('The_void', 55, 250, 0.7, 0.7, ['VoidShift'], true);
				void.setGraphicSize(Std.int(void.width * 2.5));
				void.screenCenter();
				add(void);
		
				var bgRocks:BGSprite = new BGSprite('Void_Back', -1000, -700, 0.7, 0.7);
				bgRocks.setGraphicSize(Std.int(bgRocks.width * 0.5));
				add(bgRocks);
		
				var frontRocks:BGSprite = new BGSprite('Void_Front', -1000, -600, 0.9, 0.9);
			...
			case 'a.g.o.t.i':
				...
				var bgpillar:BGSprite = new BGSprite('Pillar_BG_Stage', -1000, -700, 0.7, 0.7, ['Pillar_BG_Stage'], true);
				bgpillar.setGraphicSize(Std.int(bgpillar.width * 1.25));
				add(bgpillar);

				speaker = new BGSprite('LoudSpeaker_Moving', -650, 600, 0.9, 0.9, ['StereoMoving'], false);
			...
			case 'psychic':
				...
				if(!ClientPrefs.lowQuality) {
					var animatedBGSprite:BGSprite = new BGSprite('fireplace', 140, -340, 0.8, 0.92, ['fireplace'], true);
			...
			case 'late-drive':
				...
				asteroid.antialiasing = ClientPrefs.globalAntialiasing;
				...
				if(!ClientPrefs.lowQuality) {
					...
					voyager.antialiasing = ClientPrefs.globalAntialiasing;
			...
			case 'first-town' | 'liquated' | 'why-do-you-hate-me' | 'ghost':
				...
				var bg:BGSprite = new BGSprite('BG_CITY', -250, -100, 0.5, 0.5);
				bg.scale.set(1.55, 1.55);
				bg.active = false;
				add(bg);
	
				var wall:BGSprite = new BGSprite('BG_WALL', -250, -100, 1, 1);
				wall.scale.set(1.55, 1.55);
				wall.active = false;
				add(wall);
	
				var stage:BGSprite = new BGSprite('FG_Floor', -250, -100, 0.9, 0.9);
			...
			case 'playtime':
				...
				var bg:BGSprite = new BGSprite('factory', -500, -330, 0.9, 0.9);
				bg.active = false;
				add(bg);

				var bg2:BGSprite = new BGSprite('shadow', -500, -100, 0.5, 0.5);
				bg2.active = false;
				add(bg2);
			...
			case 'hellroll':
				...
				var bg:BGSprite = new BGSprite('bg', -90, -228, 1, 1);
				bg.active = false;
				add(bg);
				
				var rock:BGSprite = new BGSprite('rock', -32.05, 24.8, 0.6, 0.6);
				rock.active = false;
				add(rock);
				
				var ground:BGSprite = new BGSprite('ground', -488.35, 349.25, 0.8, 0.8);
				ground.active = false;
				add(ground);
				
				var glowshit:BGSprite = new BGSprite('glowshit', -105.95,-252, 0.8, 0.8);
			...
			case 'hecker':
				...
				call = new BGSprite('call', -502, -300, 0, 0);
				add(call);
		...
		switch (curStage)
		{
			case 'limo':
				BF_Y -= 220;
				BF_X += 260;
			case 'limo-corruption':
				BF_Y -= 220;
				BF_X += 260;
			case 'limo-corruption2':
				BF_Y -= 220;
				BF_X += 260;
			case 'mall':
				BF_X += 200;
		...
		switch (curStage)
		{
			...
			case 'churchruv':
				add(pillarbroke);
			case 'cg5stage':
				stageFront = new BGSprite('recordroom', -507, -117, 1, 0.9);
				stageFront.active = false;
				stageFront.setGraphicSize(Std.int(stageFront.width * 0.9));
				stageFront.updateHitbox();
				add(stageFront);

				stageFront2 = new BGSprite('lights', -507, -117, 1, 0.9);
		...
		switch (curStage)
		{
			...
			case 'garAlleyDead':
				...
				add(smoke);
			case 'mattroom':
				var frjunk:BGSprite = new BGSprite('BG_foreground junk', -600, 150, 1.2, 1.2);
				frjunk.active = false;
				add(frjunk);
			case 'mattscastle':
				var mstuff:BGSprite = new BGSprite('kingstuffidk', -940, -713, 0.9, 0.9);
				mstuff.active = false;
				add(mstuff);
		}
		...
		if(curStage == 'spooky' || curStage == 'spooky-corruption') {
			add(halloweenWhite);
		}
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				/*case 'psychic':
					...
					CoolUtil.precacheSound('bf_transform');*/
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				if(songNotes[1] > -1) { //Real notes
					...
					if (curSong == 'first-town' || curSong == 'liquated' || curSong == 'why-do-you-hate-me' || curSong == 'ghost')
					{
						daStrumTime += 30;
					}
		...
		for (i in 0...4)
		{
			...
			switch (curStage)
			{
				case 'school' | 'schoolEvil':
					...
					{
						babyArrow.loadGraphic(Paths.image('weeb/pixelUI/NOTE_assets_circle'));
					}
					else
					{
					...
					if(ClientPrefs.circleNotes)
					{
						babyArrow.loadGraphic(Paths.image('weeb/pixelUI/NOTE_assets_circle'), true, Math.floor(babyArrow.width), Math.floor(babyArrow.height));
					}
					else
					{
				...
				default:
					...
					if(ClientPrefs.circleNotes)
					{
						babyArrow.frames = Paths.getSparrowAtlas('NOTE_assets_circle');
					}
					else
					{
	...
	override public function update(elapsed:Float)
	{
		...
		switch (curStage)
		{
			...
			case 'philly' | 'philly-corruption' | 'philly-annie' | 'philly-carol':
				if (trainMoving)
				{
		...
		if (curSong == 'screenplay' || curSong == 'parasite' || curSong == 'a.g.o.t.i')
		{
			if (curSong != 'a.g.o.t.i')
			{
		...
		if(!ClientPrefs.hardcoreMode)
		{
			if(ClientPrefs.infoBar)
			{
				if(ratingString == '?')
				{
					scoreTxt.text = "Puntuacion: " + songScore + " | Fallos: " + songMisses + " | Vida: " + Math.round(health * 50) + "% | Calificacion: " + ratingString + " | Combo: " + combo;
				}
				else if (health >= 0 && ratingString == '?')
				{
					scoreTxt.text = "Puntuacion: " + songScore + " | Fallos: " + songMisses + " | Vida: " + Math.round(health * 50) + "% | Calificacion: " + ratingString + " | Combo: " + combo;
				}
				else if (health >= 0)
				{
					scoreTxt.text = "Puntuacion: " + songScore + " | Fallos: " + songMisses + " | Vida: " + Math.round(health * 50) + "% | Calificacion: " + ratingString + " (" + Math.floor(ratingPercent * 100) + "%) | Combo: " + combo;
				}
				else
				{
					scoreTxt.text = "Puntuacion: " + songScore + " | Fallos: " + songMisses + " | Vida: " + Math.round(health * 50) + "% | Calificacion: " + ratingString + " (" + Math.floor(ratingPercent * 100) + "%) | Combo: " + combo;
		...
		else if(ClientPrefs.hardcoreMode)
		{
			if(ClientPrefs.infoBar)
			{
				if(ratingString == '?')
				{
					scoreTxt.text = "Puntuacion: " + songScore + " | Vida: " + Math.round(health * 50) + "% | Calificacion: " + ratingString + " | Combo: " + combo;
				}
				else if (health >= 0 && ratingString == '?')
				{
					scoreTxt.text = "Puntuacion: " + songScore + " | Vida: " + Math.round(health * 50) + "% | Calificacion: " + ratingString + " | Combo: " + combo;
				...
				{
					scoreTxt.text = "Puntuacion: " + songScore + " | Vida: " + Math.round(health * 50) + "% | Calificacion: " + ratingString + " (" + Math.floor(ratingPercent * 100) + "%) | Combo: " + combo;
				}
				else
				{
					scoreTxt.text = "Puntuacion: " + songScore + " | Vida: " + Math.round(health * 50) + "% | Calificacion: " + ratingString + " (" + Math.floor(ratingPercent * 100) + "%) | Combo: " + combo;
		...
		if(ClientPrefs.hardcoreMode && songMisses == 1)
		{
			boyfriend.stunned = true;
	...
	public function moveCamera(isDad:Bool) {
		if(isDad) {
			...
			switch (curStage)
			{
				case 'limo' | 'limo-corruption' | 'limo-corruption2':
	...
	override function stepHit()
	{
		...
		{
			if(curStep == 2427 || curStep == 2428 || curStep == 2429 || curStep == 2430)
			{
			...
			if(curStep == 2432 || curStep == 2434 || curStep == 2435 || curStep == 2436 || curStep == 2437 || curStep == 2438 || curStep == 2439 || curStep == 2440)
			{
				dad.alpha = 0.6;
				iconP2.alpha = 0.6;
				FlxG.camera.stopFX();
			...
			if(curStep == 2943 || curStep == 2946 || curStep == 2948 || curStep == 2950 || curStep == 2452 || curStep == 2454)
			{
				dad.alpha -= 0.2;
				iconP2.alpha -= 0.2;
			}
		...
		if(ClientPrefs.camZooms)
		{
			...
			if(curSong == 'Release' && curStep == 846)
			{
				{
					if(ClientPrefs.flashing)
					{
		...
		else if(!ClientPrefs.camZooms)
		{
			...
			if(curSong == 'Release' && curStep == 847)
			{
				{
					if(ClientPrefs.flashing)
					{
		...
		if(ClientPrefs.camZooms)
		{
			if(curSong == 'Stress')
			{
				switch(curStep)
				{
					case 736 | 740 | 744 | 748 | 752 | 756 | 757:
						FlxTween.tween(FlxG.camera, {zoom: 1.5}, 1, {
							ease: FlxEase.expoOut
						});	
				}
		...
		if(curSong == 'Reflection')
		{
			switch(curStep)
			{
				...
				case 896:
					...
					FlxG.camera.shake(0.019);
				case 901:
					boyfriend.playAnim('hurt');
				case 1695:
					FlxG.sound.play(Paths.sound('yeah'));
				case 1696:
				if(SONG.player1 == 'alvaro')
				{
					boyfriend.playAnim('yeah');
				}
				else
				{
					boyfriend.playAnim('hey');
		...
		if(curSong == 'Carol-Roll')
		{
			...
			{
				case 122:
					FlxTween.tween(FlxG.camera, {zoom: 1.08}, 1, {
						ease: FlxEase.expoOut
					});
				case 124:
					var ready:FlxSprite = new FlxSprite().loadGraphic(Paths.image(introAlts[0]));
					...
					FlxTween.tween(FlxG.camera, {zoom: 1.12}, 1, {
						ease: FlxEase.expoOut
					});
				case 126:
					var set:FlxSprite = new FlxSprite().loadGraphic(Paths.image(introAlts[1]));
					...
					FlxTween.tween(FlxG.camera, {zoom: 1.16}, 1, {
						ease: FlxEase.expoOut
					});
				case 128:
					var go:FlxSprite = new FlxSprite().loadGraphic(Paths.image(introAlts[2]));
		...
		if(curSong == 'Hecker')
		{
			switch(curStep)
			{
				...
				case 1674:
					FlxTween.tween(FlxG.camera, {zoom: 1.6}, 1);
			}
		}
	}
	...
	override function beatHit()
	{
		...
		if (curStage == 'spooky' || curStage == 'spooky-corruption' && FlxG.random.bool(10) && curBeat > lightningStrikeBeat + lightningOffset)
		{
			lightningStrikeShit();
		}
		lastBeatHit = curBeat;
